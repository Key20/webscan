...
class SiteDirScan(object):
    def __init__(self, url, proxy):
        self.payloads = open(os.path.split(os.path.realpath(__file__))[0] + "\\dict\\cgi.txt").readlines()
    ...
    def site_dir_go(self):
        ...
        # ---------------------- 多线程切换 -----------------------------------------
        if IS_MULTI_THREAD is True:
            threads = [Thread(self.check_site_dir, (site_root,)) for i in range(THREAD_COUNT)]
            for i in threads:
                i.start()
            for i in threads:
                i.join()
