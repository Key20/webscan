...
class XssScan(object):
    def __init__(self, url, cookies, proxy):
        ...
        self.enc = "utf-8"
        self.urls = []
        self.payloads = json.load(open(os.path.split(os.path.realpath(__file__))[0] + "\\xss_payloads.json"))
        self.test_urls = {
            "betweenCommonTag": [],
            "betweenTitle": [],
            "betweenTextarea": [],
    ...
    def judge_location(self, url):
        try:
            ...
            re_key = re.compile(DORK_PAYLOAD)
            ...
            if soup.findAll(name="meta", attrs={"http-equiv": "Refresh", "content": re.compile(DORK_PAYLOAD)}):
                self.test_urls["inMetaRefresh"].append(url)
            
            if page_source.startswith(DORK_PAYLOAD):
                self.test_urls["utf-7"].append(url)
    
            for i in tag_list:
                for j in i.attrs:
                    if DORK_PAYLOAD in i.attrs[j]:
                        self.test_urls["inCommonAttr"].append(url)
    
                        if j in ["src", "href", "action"] and i.attrs[j].startswith(DORK_PAYLOAD):
    ...
    def test_single_payload(self, url, location, payload):
        ...
        if check_vul_sign() is True:
            return
        # test payloads
        test_url = url.replace(DORK_PAYLOAD, urllib2.quote(payload))
    ...
    def xss_go(self):
        log.output_log("[xss] test url " + self.url, True)
        test_params = self.url_parse.get_params(self.url, DORK_PAYLOAD)
        
# ---------------------- 多线程切换 -----------------------------------------
        if IS_MULTI_THREAD is True:
            threads = [Thread(self.judge_out,
                       (self.url.replace(i, test_params[i]),
                        DORK_PAYLOAD, self.urls)) for i in test_params]
            for i in threads:
                i.start()
            for i in threads:
                i.join()
            ...
            for param in test_params:
                ...
                Thread(self.judge_out, (url, DORK_PAYLOAD, self.urls))
